...
class DuckStore:
    ...
    def ingest_psc(self, data_dir: str = None):
        """
        Load PSC bulk JSONL data into the pscs table.

        PSC data is distributed as a single large JSONL file (or split).
        """
        ...
        for fpath in all_files:
            ...
            try:
                ...
                count = self.conn.execute(
                    f"SELECT count(*) FROM pscs WHERE source_file = '{fname}'"
                ).fetchone()[0]
                total += count
                logger.info(f"  Loaded {count:,} PSC records from {fname}")

            except Exception as e:
                logger.error(f"  Failed to load PSC {fname}: {e}")
